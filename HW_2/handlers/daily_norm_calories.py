...
from filters import UserWeightFilter, UserHeightFilter, UserAgeFilter, UserActionTimeFilter, UserObjCaloriesFilter
...
async def choose_weight(message: Message, 
                        state: FSMContext):
    await message.answer(text="Введите ваш вес (в кг):")
    await state.set_state(ProfileUser.user_weight)


@router.message(ProfileUser.user_weight, 
                UserWeightFilter()
                )
async def choose_height(message: Message, 
                        state: FSMContext):
...
@router.message(ProfileUser.user_height, 
                UserHeightFilter()
                )
async def choose_age(message: Message, 
                     state: FSMContext):
    # Сохранение данных состояния пользователя в переменную при помощи state.update_data
...
@router.message(ProfileUser.user_age, 
                UserAgeFilter()
                )
async def choose_action_time(message: Message, 
                             state: FSMContext):
    # Сохранение данных состояния пользователя в переменную при помощи state.update_data
...
@router.message(ProfileUser.user_action_time, 
                UserActionTimeFilter()
                )
...
async def choose_obj_calories(message: Message, 
                              state: FSMContext):
    # Сохранение данных состояния пользователя в переменную при помощи state.update_data
...
@router.message(ProfileUser.user_obj_calories,
                UserObjCaloriesFilter()
                )
async def calc_norm_calories_water(message: Message, 
                                   state: FSMContext):
    # Сохранение данных состояния пользователя в переменную при помощи state.update_data
    ...
    norm_calories = await calc_norm_calories(data['weight'],
                                             data['height'],
                                             data['age'],
                                             data['activity'])
    
    norm_water = await calc_norm_water(data['weight'],
                                       data['activity'],
                                       data['city'])

    await message.answer(text=f"Ваша суточная норма воды: {norm_water} мл. \nВаша суточная норма каллорий: {norm_calories} ккал.")
