...
from states.state_user import LoggingFood
...
async def choose_product(message: Message, command: CommandObject, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"{input_food.title()} — {data_food['foods'][0]['nf_calories']} ккал на {data_food['foods'][0]['serving_weight_grams']} г. Сколько грамм вы съели?"
        )
        # Заносим данные состояний в словарь
        await state.update_data(food_name=input_food)
        await state.update_data(
            food_calories=float(data_food["foods"][0]["nf_calories"])
        )
        await state.update_data(
            food_serving_weight_grams=float(
                data_food["foods"][0]["serving_weight_grams"]
            )
        )
        # Меняем состояние
        ...
        await message.answer(
            "Ошибка. Данного продукта не существует или продукт введен неверно. \nВведите название продукта на <b>английском языке</b> в следующем формате: \n/log_food <i>product</i>"
        )


@router.message(LoggingFood.food_eat_weight, UserFoodWeightFilter())
async def choose_food_eat_weight(message: Message, state: FSMContext):
    ...
    logged_calories = (
        data["food_calories"] / data["food_serving_weight_grams"]
    ) * data["food_eat_weight"]
...
async def choose_workout_info(
    ...
    try:
        ...
        await message.answer(f"{await state.get_data()}")
        await message.answer(
            f"{workout_type.lower().title()} {workout_time} минут — {burned_calories} ккал. Дополнительно: выпейте {_drinked_water} мл воды."
        )
